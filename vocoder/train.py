...
def train(run_id: str, syn_dir: Path, voc_dir: Path, models_dir: Path, ground_truth: bool,
    ...
    vocoder_weights_fpath = model_dir.joinpath(run_id + ".pt")
    weights_fpath = 'vocoder/saved_models/pretrained/pretrained.pt'

    if force_restart: # or not weights_fpath.exists()
        print("\nStarting the training of WaveRNN from scratch\n")
        model.save(vocoder_weights_fpath, optimizer)
    else:
    ...
    for epoch in range(1, hp.n_epochs):
        ...
        for i, (x, y, m) in enumerate(data_loader, 1):
            ...
            # Forward pass
            y_hat = model(x, m)  # instansiating the model
            if model.mode == 'RAW':
